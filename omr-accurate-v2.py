...
class AccurateOMRProcessor:
    def __init__(self):
        ...
        self.area_range = (10, 12000)
        self.fill_threshold = 0.05
        self.min_circularity = 0.08
        self.max_aspect_deviation = 1.6
    ...
    def detect_bubbles(self, image):
        ...
        # Visual overlay on debug image for rows and bubbles
        debug_img = cv2.cvtColor(gray, cv2.COLOR_GRAY2BGR)
        for i, row in enumerate(rows):
            color = (0, 255 - i * 20, i * 20)  # Different color per row
            for idx, (x, y, w, h) in enumerate(row):
                cv2.rectangle(debug_img, (x, y), (x + w, y + h), color, 2)
                cv2.putText(debug_img, f"{idx + 1}", (x, y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)
            cv2.putText(debug_img, f"Row {i + 1} - {len(row)} bubbles", (10, 20 + i * 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)

        st.image(debug_img, caption="Visual Bubble Detection with Rows", use_column_width=True)
